...
- **グローバル変数を原則禁止する。Python・C++ 問わず。** 定数 (`const` / 大文字スネークケース) も含む。値は引数・戻り値・構造体・クラスフィールドで受け渡すこと。
  - **例外**: import されることを前提とするモジュールで、設計上妥当と判断される場合は許可する。具体例: ctypes ラッパーにおけるライブラリハンドル (`_lib`) やポインタ型エイリアス (`_dp`, `_ip`) のようなモジュールレベル変数。
