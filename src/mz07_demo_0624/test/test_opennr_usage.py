...
def send_to_robot(mid_node_robot: np.ndarray) -> None:
    ...
    print("ctrl move x xt")
    print(f"x {mid_node_robot[0]}, y {mid_node_robot[1]}, z {mid_node_robot[2]}")
...
p_h: np.ndarray = np.array([100, 100, 0, 1.0], dtype=np.float64)  # mm
#mid_robot: np.ndarray = (T_camera_to_robot @ p_h)[:3]
mid_robot: np.ndarray = p_h

print("send target sig tool")
