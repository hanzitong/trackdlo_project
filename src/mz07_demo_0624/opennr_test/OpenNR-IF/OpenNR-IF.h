...
#define		NR_ACCESS_NO_WAIT		1				// No WaitÅj
...
// For CtrlCheckGo, CtrlCheckBack
#define		NR_CHECK_STOP_REQ		0				// Check stop
#define		NR_CHECK_START_REQ		1				// Check start
...
//------------------------------------------------
// SubId MAXÅi For Set()/Get()Åj
//------------------------------------------------
#define		NR_MAX_UNIT				(9)
#define		NR_MAX_MECHANISM		(9)
...
#define		NR_E_UDATE_OLDFILE		-3006				// cannnot modify because old file
...
#define		NR_E_CTRL_EXECREJECT	-4005				// exec	reject error
#define		NR_E_CTRL_EXECBANNED	-4006				// exec banned error 

#define		NR_E_CTRL_TABLETTP		-4100				// tablet TP connect error
...
// Get Pose Type ID
//------------------------------------------------
#define POSETYPE_NOSELECT	(0)
#define POSETYPE_XYZ		(1)
#define POSETYPE_ANG		(2)

//------------------------------------------------
// Compile File Type ID
//------------------------------------------------
#define FILETYPE_PROGRAM	(0)
#define FILETYPE_USERTASK	(1)
#define FILETYPE_POSEFILE	(2)
#define FILETYPE_GLOBALFUNC	(3)

//------------------------------------------------
// device type ID
//------------------------------------------------
#define DEVICE_TYPE_INTERNAL_MEMORY (0)
#define DEVICE_TYPE_EXTERNAL_DEVICE (1)

//------------------------------------------------
// PLC boot status ID
//------------------------------------------------
#define		PLC_STATUS_DISABLE		(0)
#define		PLC_STATUS_STOP			(1)
#define		PLC_STATUS_RUN			(2)
...
typedef struct ConfigData{
		...
		USHORT			ushJ6	 	: 3; 		// 4:-180Å`180 / 5:0Å`360 / 6:-360Å`0
		USHORT			unRsv2		: 4;		// Rsv
...
#define NR_CLASSIC_MODE_ID_MAX (99)
#define NR_CLASSIC_MODE_ID_MIN (1)
#define NR_CLASSIC_MODE_COMMENT_MAX (20)

typedef struct NR_ClassicModeIpConfig{
	BOOL bDeleteConfig;
	BOOL bOpeMode;
	int nID;
	int nIPaddr[4];
	char chComment[NR_CLASSIC_MODE_COMMENT_MAX + 1];
} NR_CLASSIC_MODE_IPCONFIG;

typedef struct NR_ClassicModeConfig {
	BOOL bEnabledWebTP;
	NR_CLASSIC_MODE_IPCONFIG stIpConfig[NR_CLASSIC_MODE_ID_MAX];
} NR_CLASSIC_MODE_CONFIG;
...
#ifdef _UNICODE
#define NR_FILEDATA NR_FILEDATAW
#else
#define NR_FILEDATA NR_FILEDATAA
#endif

typedef struct NR_FileDataA {
	char	chFileName[FILE_NAME_MAX+1];
	int		nFileType;
} NR_FILEDATAA;

typedef struct NR_FileDataW {
	wchar_t	chFileName[FILE_NAME_MAX+1];
	int		nFileType;
} NR_FILEDATAW;

typedef struct NR_ComVersion {
	int nControllerType;
	int nComType;
	int nVersion;
	int nSubVersion;
 ...
 *	@param[in]	BOOL bIsAllowedDuplicate: Allow to create same connection object
 *	@param[out]	none	
 *	@retval		SUCCESS		connect object id
 *				FAIL		Error Code
 */
NR_EXTENDED_DLL_API int NR_Open( NACHI_COMMIF_INFO *pInfo, BOOL bIsAllowedDuplicate = FALSE);
...
NR_EXTENDED_DLL_API int NR_AcsGeneralInputSignal(int nOpenId, BOOL value[], int nSubId, int nCount,
													int nPriority = NR_PULL_MODE, float fThreshold = 0.01f, BOOL bUpdate = FALSE);
 ...
 *					FAIL:NR_E_XXXXX
 */
NR_EXTENDED_DLL_API int NR_AcsPoseValue(int nOpenId, NR_POSE* value, BOOL bUpdate, int nPoseFileNo, int nPoseNo, int nUnitId = 1,
													int nPriority = NR_PULL_MODE, float fThreshold = 0.01f);
 ...
 *					FAIL:NR_E_XXXXX
 */
NR_EXTENDED_DLL_API int NR_AcsPoseValueConfig(int nOpenId, NR_POSE_CONF* value, BOOL bUpdate, int nPoseFileNo, int nPoseNo, int nUnitId = 1,
													int nPriority = NR_PULL_MODE, float fThreshold = 0.01f);
...
/*!	@brief			Get controller communication version
 *	@param[in]		nOpenId			connect object id
 *	@param[out]		value			controller communication version
 *	@retval			SUCCESS:NR_E_NORMAL
 *					FAIL:NR_E_XXXXX
 */
NR_EXTENDED_DLL_API int NR_AcsComVersion(int nOpenId, NR_COMVERSION* value);
...
NR_EXTENDED_DLL_API int NR_AcsOpenNRVersion(DWORD* pdwMainVer, DWORD* pdwSubVer1, DWORD* pdwSubVer2, DWORD* pdwSubVer3);
