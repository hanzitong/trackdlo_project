...
SAFE_Y_MAX: float = -170
SAFE_Y_MIN: float = 360
...
def is_safe_xyz(x: float, y: float, z: float) -> bool:
    ...
    if(not is_safe): print("not safe xyz !!!!!!!")
...
def main() -> None:
    ...
    try:
        ...
        if len(X) < NUM_NODES * 3:
            print(f"not enough points: {len(X)}")
            return

        # ── Step 4: TrackDLO で初期化 ─────────────────────────────────────────
        state: "TrackdloState | None" = initialize_state(
            np.ascontiguousarray(X, dtype=np.float64)
        )
        if state is None:
            print("TrackDLO initialization failed")
            return
