# =============================================================================
# train.py  (scripts_aoyama_halfsize/)
#
# DeepLabV3+ + MobileNetV2 エンコーダーで 320×240 画像を使って学習する。
#
# 入力データ: data_aoyama_halfsize/dataset/ (my_halfsize_code.py で生成)
# 出力重み:   weights_halfsize/
#   best_deeplabv3plus_mobilenetv2.pth  ← val IoU が最良のエポック
#   last_deeplabv3plus_mobilenetv2.pth  ← 最終エポック
#
# 使い方 (ワークスペースルートから):
#   uv run python src/bmask_gen/scripts_aoyama_halfsize/train.py
# =============================================================================
...
WEIGHTS_DIR: Path  = ROOT / "weights_halfsize"
BEST_PATH: Path    = WEIGHTS_DIR / "best_deeplabv3plus_mobilenetv2.pth"
LAST_PATH: Path    = WEIGHTS_DIR / "last_deeplabv3plus_mobilenetv2.pth"
...
class CableDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # infer_live.py / hailo_halfsize/infer_live.py も同じ正規化を使うこと
        image = image.astype(np.float32) / 255.0

        # HWC → CHW: (240,320,3) → (3,240,320)
...
def main() -> None:
    ...
    # ─── モデル ───────────────────────────────────────────────────────────
    # DeepLabV3+ (V3+): ASPP + デコーダースキップ接続あり。
    # エンコーダーに mobilenet_v2 を使うことで ResNet34 より軽量。
    # 320×240 入力ならシングルコンテキストに収まる見込み。
